...
const createTracingMixins = (options) => {
  ...
  for (const operation of hooks) {
    ...
    if (!internalHooks) {
      debugBuild.DEBUG_BUILD && utils.logger.warn(`Unknown hook: ${operation}`);
      continue;
    }
    ...
    for (const internalHook of internalHooks) {
      mixins[internalHook] = function () {
        ...
        if (internalHook == internalHooks[0]) {
          const activeSpan = (this.$root && this.$root.$_sentryRootSpan) || browser.getActiveSpan();
          if (activeSpan) {
            ...
            this.$_sentrySpans[operation] = browser.startInactiveSpan({
              name: `Vue <${name}>`,
              op: `${VUE_OP}.${operation}`,
              origin: 'auto.ui.vue',
            });
